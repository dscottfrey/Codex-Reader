...
### 3.7 Inline Image Zoom — Lightbox

**Spec lives in `01.1_RENDERING_ADDENDUM.md §A1`.** Full design: hero animation from tap position, Photos-style pinch/zoom, swipe-down to dismiss with return animation, caption from `<figcaption>` only. No gallery mode — firm design decision, not a deferred feature.
